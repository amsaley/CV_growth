...
```{r}
library(dplyr)
sa.proc = sa.proc[sa.proc$dps == 0 | sa.proc$dps == 15 | sa.proc$dps == 36,]
...
growth_rate = sa.proc %>%
  ...
  mutate(Diff_time = date.numb - lag(date.numb),  # Difference in time (just in case there are gaps)
         ...
         Net_growth = SA.mm2 - SA.start,
         incr_growth = (Diff_growth/Diff_time)# growth rate in percent 
         )
...
dat = full[full$SA.start < 250 & full$Net_growth > -15 & full$`%oc.ic.shell` > 0,]
...
```
what is the effect of TA on growth?
```{r}
amb.S = dat[dat$sal.treat.x == "amb",]

#after 5 weeks of exposure
end = amb.S[amb.S$dps == 36,]
mod = aov(Net_growth ~ factor(TA.treat.x), data=end, na.action=na.omit)
TukeyHSD(mod, factor("end$TA.treat.x"))
plot(TukeyHSD(mod, factor("end$TA.treat.x")))

boxplot(amb.S[amb.S$dps == 36,]$Net_growth ~ amb.S[amb.S$dps == 36,]$TA.treat.x, pch = 19, ylab = NA, xlab = NA, names= NA, ylim = c(-20,60), frame.plot = F, xlim = c(0,4), axes = F, col = "grey40")
lines(x= c(0,4), y=c(-20,-20), lwd = 2)
lines(x= c(0,0), y=c(-20,60), lwd = 2)
axis(side = 1, line = -1, tick = F, at = c(1,2,3), labels = c("reduced","moderate","elevated"), cex.axis = 1.25)
axis(side = 1, line = .5, tick = F, at = 2, labels = "Alkalinity Level", cex.axis = 1.25)
axis(side = 2, line = -1.5, tick = F, at = c(-20,-10,0,10,20,30,40,50,60), labels = c("-20","-10","0","10","20","30","40","50","60"), cex.axis = 1.25, las = 1)
axis(side = 2, line = .25, tick = F, at = 20, labels = expression(paste('Net surface area growth (mm'^2,')')), cex.axis = 1.25)


#plot(amb.S[amb.S$dps == 36,]$Net_growth ~ jitter(amb.S[amb.S$dps == 36,]$med.ta,20), pch = 19)
#summary(lm(amb.S[amb.S$dps == 36,]$Net_growth ~ amb.S[amb.S$dps == 36,]$med.ta))
#abline(5.6961422,0.0025330, lwd = 3)
...
#after 5 weeks of exposure
boxplot(low.S[low.S$dps == 36,]$Net_growth ~ low.S[low.S$dps == 36,]$TA.treat.x, pch = 19, ylab = NA, xlab = NA, names= NA, ylim = c(-20,60), frame.plot = F, xlim = c(0,4), axes = F, col = "grey40")
lines(x= c(0,4), y=c(-20,-20), lwd = 2)
lines(x= c(0,0), y=c(-20,60), lwd = 2)
axis(side = 1, line = -1, tick = F, at = c(1,2,3), labels = c("Rain","low TA","high TA"), cex.axis = 1.25)
axis(side = 1, line = .5, tick = F, at = 2, labels = "Simulated FW source", cex.axis = 1.25)
axis(side = 2, line = -1.5, tick = F, at = c(-20,-10,0,10,20,30,40,50,60), labels = c("-20","-10","0","10","20","30","40","50","60"), cex.axis = 1.25, las = 1)
axis(side = 2, line = .25, tick = F, at = 20, labels = expression(paste('Net surface area growth (mm'^2,')')), cex.axis = 1.25)
end = low.S[low.S$dps == 36,]
mod = aov(Net_growth ~ factor(TA.treat.x), data=end, na.action=na.omit)
TukeyHSD(mod, factor("end$TA.treat.x"))
plot(TukeyHSD(mod, factor("end$TA.treat.x")))

```
do the effects of TA and S appear as 'shock effects' (ie they disappear after two weeks) or do they persist? need to look at incremental growth here...
```{r}
dat = dat[dat$dps != 0,]
#amb salinity
amb.S = amb.S[amb.S$dps != 0,]
#incremental growth rates decline in amb TA conditions, suggesting that maybe the effect of altered
boxplot(amb.S$incr_growth ~ amb.S$dps : amb.S$TA.treat.x, pch = 19, ylab = NA, xlab = NA, col = unique(c("grey","grey40")), names= NA, ylim = c(-1.5,3.5), frame.plot = F, xlim = c(0,9), axes = F)
text(x = 8, y = 2.8, labels = c("Exposure period"), cex = 1.25)
legend(x=6.6,y=2.8,fill = c("grey","grey40"),legend = c("days 0-15","days 15-36"),bty = "n", cex = 1.2, x.intersp=0.4)
lines(x= c(0,7), y=c(-1.5,-1.5), lwd = 2)
lines(x= c(0,0), y=c(-1.5,3.5), lwd = 2)
axis(side = 1, line = -1, tick = F, at = c(1.5,3.5,5.5), labels = c("Reduction","Control","Increase"), cex.axis = 1.25)
axis(side = 1, line = .25, tick = F, at = 3.5, labels = "Alkalinity Level", cex.axis = 1.25)
axis(side = 2, line = -1.5, tick = F, at = c(-1,0,1,2,3), labels = c("-1.0","0.0","1.0","2.0","3.0"), cex.axis = 1.25, las = 1)
axis(side = 2, line = 1.75, tick = F, at = 1, labels = "Surface area growth rate", cex.axis = 1.25)
axis(side = 2, line = .25, tick = F, at = 1, labels = expression(paste('(mm'^2,' day'^-1,')')), cex.axis = 1.25)


low.S = low.S[low.S$dps != 0,]
boxplot(low.S$incr_growth ~ low.S$dps : low.S$TA.treat.x, pch = 19, ylab = NA, xlab = NA, col = unique(c("grey","grey40")), names= NA, ylim = c(-1.5,4.5), frame.plot = F, xlim = c(0,9), axes = F)
text(x = 8, y = 2.8, labels = c("Exposure period"), cex = 1.25)
legend(x=6.6,y=2.8,fill = c("grey","grey40"),legend = c("days 0-15","days 15-36"),bty = "n", cex = 1.2, x.intersp=0.4)
lines(x= c(0,7), y=c(-1.5,-1.5), lwd = 2)
lines(x= c(0,0), y=c(-1.5,4.5), lwd = 2)
axis(side = 1, line = -1, tick = F, at = c(1.5,3.5,5.5), labels = c("Rain","Low TA","High TA"), cex.axis = 1.25)
axis(side = 1, line = .25, tick = F, at = 3.5, labels = "Simulated FW source", cex.axis = 1.25)
axis(side = 2, line = -1.5, tick = F, at = c(-1,0,1,2,3,4), labels = c("-1.0","0.0","1.0","2.0","3.0","4.0"), cex.axis = 1.25, las = 1)
axis(side = 2, line = 1.75, tick = F, at = 1.5, labels = "Surface area growth rate", cex.axis = 1.25)
axis(side = 2, line = .25, tick = F, at = 1.5, labels = expression(paste('(mm'^2,' day'^-1,')')), cex.axis = 1.25)
mod = aov(incr_growth ~ factor(TA.treat.x) : factor(dps), data=low.S, na.action=na.omit)
TukeyHSD(mod, factor("TA.treat.x:dps"))
plot(TukeyHSD(mod, factor("TA.treat.x:dps")))
```
shell properties + CI
```{r}
plot(amb[amb$dps == 36,]$SA.mm2 ~ amb[amb$dps == 36,]$gut.wt, pch = 19, col = as.factor(amb[amb$dps == 36,]$sal.treat.x), ylab = "Surface area of shell (mm2)", xlab = "Dry tissue mass (mg)")
summary(lm(amb[amb$dps == 36,]$SA.mm2 ~ amb[amb$dps == 36,]$gut.wt + factor(amb[amb$dps == 36,]$sal.treat.x)))
abline(52.8442, 13.2406, col = "black", lwd = 3)
abline(52.5430+24.0465, 13.2406, col = "red", lwd = 3)
legend(x = 13, y = 125, legend = c("low", "ambient"), fill = c("red","black"), bty = "n")
text(x = 14.5, y = 125, labels = "Salinity")
